...
for i in $SYS; do
    ...
    env GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -ldflags "$COMMITFLAGS" -tags="signrpc walletrpc chainrpc invoicesrpc" github.com/monasuite/lnd/cmd/lnd
    env GOOS=$OS GOARCH=$ARCH GOARM=$ARM go build -v -ldflags "$COMMITFLAGS" -tags="invoicesrpc" github.com/monasuite/lnd/cmd/lncli
    cd ..
