...
func TestMigrateInvoices(t *testing.T) {
	...
	var monaNetParams bitcoinCfg.Params
	monaNetParams.Bech32HRPSegwit = monacoinCfg.MainNetParams.Bech32HRPSegwit
	payReqMona, err := getPayReq(&monaNetParams)
	if err != nil {
		t.Fatal(err)
	}
	...
	invoices := []Invoice{
		...
		{
			PaymentRequest: []byte(payReqMona),
		},
	}
