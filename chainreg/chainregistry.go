...
func NewChainControl(cfg *Config, blockCache *blockcache.BlockCache) (
	*ChainControl, func(), error) {
	...
	if cfg.PrimaryChain() == MonacoinChain {
	...
	case "bitcoind", "monacoind":
		...
		if strings.Contains(bitcoindMode.RPCHost, ":") {
			...
			if (cfg.Bitcoin.Active &&
				...
				(cfg.Monacoin.Active && cfg.Monacoin.RegTest) {
				...
				if err != nil || conn == nil {
					...
					case cfg.Bitcoin.Active && cfg.Bitcoin.RegTest:
						rpcPort = 18443
					case cfg.Monacoin.Active && cfg.Monacoin.RegTest:
						rpcPort = 19443
		...
		if cfg.Bitcoin.Active && !cfg.Bitcoin.RegTest {
			...
			log.Infof("Initializing monacoind backed fee estimator in "+
				"%s mode", bitcoindMode.EstimateMode)
